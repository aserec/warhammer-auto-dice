...
test("US6: toggling hit bonus updates summary", async ({ page }) => {
  ...
  await expect(page.locator("p").filter({ hasText: /^Modifiers$/ }).first()).toBeVisible();
  await page.getByRole("button", { name: /hit bonus/i }).click();
  await expect(page.getByText(/hit bonus/)).toBeVisible();
});
